package dev.havoc.havoctab.shared.features.layout.impl.common;
...
import dev.havoc.havoctab.shared.features.layout.pattern.FixedSlotPattern;
...
public class FixedSlot extends RefreshableFeature {
    ...
    public static FixedSlot fromDefinition(@NotNull FixedSlotPattern def, @NotNull LayoutPattern pattern, @NotNull LayoutManagerImpl manager) {
        ...
        if (!def.getText().isEmpty()) HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.layoutSlot(pattern.getName(), def.getSlot()), f);
        return f;
    }
    ...
    private int parsePing(@NotNull String value, @NotNull TabPlayer viewer) {
        ...
        try {
            ...
            if (pingText != null && pingText.contains("%")) {
                HavocTab.getInstance().getConfigHelper().runtime().error(String.format(
