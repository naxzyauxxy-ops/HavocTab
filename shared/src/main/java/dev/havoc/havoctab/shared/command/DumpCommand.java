...
import dev.havoc.havoctab.shared.hook.MiniMessageHook;
...
public class DumpCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        if (args.length > 0) {
            TabPlayer analyzed = HavocTab.getInstance().getPlayer(args[0]);
            if (analyzed == null) {
    ...
    private void dump(@Nullable TabPlayer sender, @NotNull TabPlayer analyzed) {
        ...
        data.put("platform", HavocTab.getInstance().getPlatform().dump());
        data.put("player", analyzed.dump());
        data.put("features", dumpFeatures(analyzed));
        data.put("minimessage", dumpMiniMessage());
        data.put("placeholders", HavocTab.getInstance().getPlaceholderManager().dump(analyzed));
        ...
        try {
            ...
            HavocTab.getInstance().getErrorManager().criticalError("Failed to upload dump", e);
        }
    }
    ...
    private Map<String, Object> dumpFeatures(@NotNull TabPlayer player) {
        ...
        for (String featureName : featureList) {
            TabFeature feature = HavocTab.getInstance().getFeatureManager().getFeature(featureName);
    ...
    private Map<String, Object> dumpMiniMessage() {
        ...
        content.put("support is enabled in config", HavocTab.getInstance().getConfiguration().getConfig().getComponents().isMinimessageSupport());
        ...
        content.put("HavocTab's MiniMessage instance", String.valueOf(MiniMessageHook.getMiniMessage()));
    ...
    private Map<String, Object> dumpFiles() {
        ...
        Configs configs = HavocTab.getInstance().getConfiguration();
