...
public class ModernConverter {
    ...
    public ModernConverter() {
        ...
        converters.put(8, config -> {
            ...
            Map<String, Object> mentionSound = new LinkedHashMap<>();
            mentionSound.put("enabled", true);
            mentionSound.put("name", "ENTITY_EXPERIENCE_ORB_PICKUP");
            mentionSound.put("volume", 1.0);
            mentionSound.put("pitch", 1.2);

            Map<String, Object> mentionToggle = new LinkedHashMap<>();
            mentionToggle.put("enabled", true);
            mentionToggle.put("toggle-command", "/mentions");
            mentionToggle.put("remember-toggle-choice", true);
            mentionToggle.put("hidden-by-default", false);

            Map<String, Object> mentions = new LinkedHashMap<>();
            mentions.put("enabled", true);
            mentions.put("require-at-symbol", false);
            mentions.put("highlight", "&e&l%player%");
            mentions.put("action-bar", "&e%player% &fmentioned you");
            mentions.put("sound", mentionSound);
            mentions.put("toggle", mentionToggle);
            ...
            chat.put("mentions", mentions);

            Map<String, Object> spam = new LinkedHashMap<>();
            spam.put("enabled", true);
            spam.put("cooldown-milliseconds", 1500);
            spam.put("block-repeats", true);
            spam.put("repeat-history", 3);
            spam.put("max-caps-percent", 70);
            spam.put("caps-min-length", 8);
            spam.put("max-repeated-characters", 4);

            Map<String, Object> blockedWords = new LinkedHashMap<>();
            blockedWords.put("enabled", true);
            blockedWords.put("action", "BLOCK");
            blockedWords.put("censor-replacement", "***");
            blockedWords.put("words", new ArrayList<String>());
            blockedWords.put("allowed-phrases", new ArrayList<String>());

            Map<String, Object> advertising = new LinkedHashMap<>();
            advertising.put("enabled", true);
            advertising.put("action", "BLOCK");
            advertising.put("block-ips", true);
            advertising.put("block-domains", true);
            advertising.put("block-discord-invites", true);
            advertising.put("domain-pattern", "\\b[a-z0-9][a-z0-9-]*\\.(?:com|net|org|gg|xyz|io|me|tv|club|store|shop|fun|online|site|pro|co|uk|eu|ru|de|nl|ca|au|info|biz|link|live|cc|to|ly|sh|dev|app|mc|us)\\b");
            advertising.put("allowed", new ArrayList<String>());

            Map<String, Object> filter = new LinkedHashMap<>();
            filter.put("enabled", true);
            filter.put("bypass-permission", "havoctab.chat.bypassfilter");
            filter.put("spam", spam);
            filter.put("blocked-words", blockedWords);
            filter.put("advertising", advertising);

            chat.put("filter", filter);

            config.set("chat", chat);
        });
        converters.put(9, config -> {
            // Retire allow-self-mention: self-mentions are now always ignored, so leaving a
            // dead "true" in the file would be a setting that silently does nothing.
            config.set("chat.mentions.allow-self-mention", null);

            // Reword the mention action bar, but ONLY if it still holds the old shipped
            // default. Anyone who customised the line keeps exactly what they wrote.
            Object actionBar = config.getObject("chat.mentions.action-bar");
            if ("&#f40d0d%player% &fmentioned you".equals(actionBar)) {
                config.set("chat.mentions.action-bar", "&fYou have been mentioned by &#f40d0d%player%");
            }
        });
    }
