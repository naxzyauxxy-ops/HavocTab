...
import dev.havoc.havoctab.shared.features.sorting.types.SortingType;
...
public class StartupWarnPrinter {
    ...
    public void startupWarn(@NonNull File file, @NonNull String message) {
        warnCount++;
        HavocTab.getInstance().getPlatform().logWarn(new TabTextComponent("[" + file.getName() + "] " + message, TabTextColor.RED));
    }
    ...
    public void printWarnCount() {
        ...
        HavocTab.getInstance().getPlatform().logWarn(new TabTextComponent("Found a total of " + warnCount + " issues.", TabTextColor.RED));
