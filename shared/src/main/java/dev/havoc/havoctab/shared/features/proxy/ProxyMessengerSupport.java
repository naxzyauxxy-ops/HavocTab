...
public class ProxyMessengerSupport extends ProxySupport {
    ...
    public void register() {
        try {
            ...
            messenger.start();
            HavocTab.getInstance().getPlatform().logInfo(new TabTextComponent("Successfully connected to " + messengerName, TabTextColor.GREEN));
        } catch (Exception e) {
            HavocTab.getInstance().getErrorManager().criticalError("Failed to connect to " + messengerName + ": " + e.getClass().getName() + ": " + e.getMessage(), null);
        }
    }
