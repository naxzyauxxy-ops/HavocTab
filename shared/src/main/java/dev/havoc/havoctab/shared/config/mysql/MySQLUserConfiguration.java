...
public class MySQLUserConfiguration implements PropertyConfiguration {
    ...
    public void remove(@NonNull String player) {
        try {
            ...
            HavocTab.getInstance().getErrorManager().mysqlQueryFailed(e);
        }
    ...
    private TabPlayer getPlayer(@NonNull String string) {
        TabPlayer p = HavocTab.getInstance().getPlayer(string);
        if (p == null) {
            try {
                p = HavocTab.getInstance().getPlayer(UUID.fromString(string));
        ...
        HavocTab.getInstance().getCPUManager().getMysqlThread().execute(() -> {
            ...
            try {
                ...
                while (crs.next()) {
                    ...
                    HavocTab.getInstance().debug("Loaded user line: " + String.format("%s, %s, %s, %s, %s", user, property, value, world, server));
                ...
                HavocTab.getInstance().debug("Loaded MySQL data of " + player.getName());
