...
import dev.havoc.havoctab.shared.config.files.Animations;
import dev.havoc.havoctab.shared.config.files.Config;
import dev.havoc.havoctab.shared.config.mysql.MySQL;
import dev.havoc.havoctab.shared.config.mysql.MySQLGroupConfiguration;
import dev.havoc.havoctab.shared.config.mysql.MySQLUserConfiguration;
import dev.havoc.havoctab.shared.config.skin.SkinManager;
...
public class Configs {
    ...
    private final YamlPropertyConfigurationFile groupsFile = new YamlPropertyConfigurationFile(
            ...
            new File(HavocTab.getInstance().getDataFolder(), "groups.yml")
    );
    ...
    private final YamlPropertyConfigurationFile usersFile = new YamlPropertyConfigurationFile(
            ...
            new File(HavocTab.getInstance().getDataFolder(), "users.yml")
    );
    ...
    private final ConfigurationFile playerData = new YamlConfigurationFile(
            ...
            new File(HavocTab.getInstance().getDataFolder(), "playerdata.yml")
    );

    /** Skin cache file for Layout and Head components */
    private final ConfigurationFile skinCache = new YamlConfigurationFile(
            new ByteArrayInputStream(new byte[0]),
            new File(HavocTab.getInstance().getDataFolder(), "skincache.yml")
    );
    ...
    public Configs() throws IOException {
        File errorLog = HavocTab.getInstance().getErrorManager().getErrorLog();
        if (errorLog.length() > TabConstants.MAX_LOG_SIZE) {
            HavocTab.getInstance().getConfigHelper().startup().startupWarn(errorLog, "The file has reached its size limit (16MB). No new errors will be logged. " +
        ...
        if (config.getMysql() != null) {
            try {
                ...
                HavocTab.getInstance().getErrorManager().mysqlConnectionFailed(e);
            }
        }
        groups = groupsFile;
        users = usersFile;
