...
import dev.havoc.havoctab.shared.features.scoreboard.ScoreRefresher;
...
public abstract class ScoreboardLine extends RefreshableFeature implements Line, CustomThreaded {
    ...
    protected ScoreboardLine(@NonNull ScoreboardImpl parent, int lineNumber, @NotNull String text) {
        initializeText(text);
        this.parent = parent;
        teamName = "HTab-Sidebar-" + lineNumber;
        ...
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.scoreboardScore(parent.getName(), lineNumber), scoreRefresher);
    }
