...
import dev.havoc.havoctab.shared.placeholders.PlaceholderIdentifier;
...
import dev.havoc.havoctab.shared.placeholders.PlaceholderRefreshTask;
import dev.havoc.havoctab.shared.placeholders.conditions.ConditionManager;
...
import dev.havoc.havoctab.shared.placeholders.types.ServerPlaceholderImpl;
import dev.havoc.havoctab.shared.placeholders.types.TabPlaceholder;
...
public class PlaceholderManagerImpl extends RefreshableFeature implements PlaceholderManager, JoinListener, Loadable, Dumpable {
    ...
    public PlaceholderManagerImpl(@NotNull CpuManager cpu, @NotNull PlaceholderRefreshConfiguration configuration) {
        ...
        tabExpansion = HavocTab.getInstance().getConfiguration().getConfig().getPlaceholders().isRegisterTabExpansion() ?
                HavocTab.getInstance().getPlatform().createTabExpansion() : new EmptyTabExpansion();
    }
    ...
    private void processRefreshResults(@NotNull PlaceholderRefreshTask task) {
        ...
        for (RefreshableFeature f : updateServerPlaceholders(task.getServerPlaceholderResults())) {
            update.put(f, new HashSet<>(HavocTab.getInstance().getData().values()));
        }
    ...
    public static List<String> detectPlaceholders(@NonNull String text) {
        ...
        detectedPlaceholders.addAll(HavocTab.getInstance().getPlatform().detectAdditionalPlaceholders(text));
    ...
    public PlayerPlaceholderImpl registerPlayerPlaceholder(@NonNull String identifier,
                                                           @NonNull Function<dev.havoc.havoctab.api.TabPlayer, String> function) {
    ...
    public RelationalPlaceholderImpl registerRelationalPlaceholder(
            ...
            @NonNull BiFunction<dev.havoc.havoctab.api.TabPlayer, dev.havoc.havoctab.api.TabPlayer, String> function
    ) {
    ...
    public void registerPlayerPlaceholder(@NonNull Pattern identifier, int refresh,
                                          @NonNull Function<Matcher, Function<dev.havoc.havoctab.api.TabPlayer, String>> function) {
        ensureActive();
    ...
    public void registerRelationalPlaceholder(@NonNull Pattern identifier, int refresh,
                                              @NonNull Function<Matcher, BiFunction<dev.havoc.havoctab.api.TabPlayer, dev.havoc.havoctab.api.TabPlayer, String>> function) {
        ensureActive();
    ...
    public synchronized PlaceholderReference getPlaceholderReference(@NonNull String identifier) {
        ...
        HavocTab.getInstance().getPlatform().registerUnknownPlaceholder(identifier);
