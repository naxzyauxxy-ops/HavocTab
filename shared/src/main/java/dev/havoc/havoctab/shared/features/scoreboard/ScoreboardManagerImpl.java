package dev.havoc.havoctab.shared.features.scoreboard;

import lombok.Getter;
import lombok.NonNull;
import dev.havoc.havoctab.api.scoreboard.Line;
...
import dev.havoc.havoctab.shared.features.scoreboard.ScoreboardConfiguration.ScoreboardDefinition;
import dev.havoc.havoctab.shared.features.scoreboard.lines.ScoreboardLine;
...
public class ScoreboardManagerImpl extends RefreshableFeature implements ScoreboardManager, JoinListener,
    ...
    public static final String OBJECTIVE_NAME = "HTab-Scoreboard";

    private final StringToComponentCache cache = new StringToComponentCache("Scoreboard", 1000);

    private final ThreadExecutor customThread = new ThreadExecutor("HavocTab Scoreboard Thread");
    ...
    @Nullable
    private dev.havoc.havoctab.api.scoreboard.Scoreboard announcement;
    ...
    public ScoreboardManagerImpl(@NotNull ScoreboardConfiguration configuration) {
        ...
        if (configuration.isRememberToggleChoice()) {
            toggleManager = new ToggleManager(HavocTab.getInstance().getConfiguration().getPlayerData(), "scoreboard-off");
        }
    }
    ...
    public void load() {
        ...
        for (Entry<String, ScoreboardDefinition> entry : configuration.getScoreboards().entrySet()) {
            ...
            HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.scoreboardLine(scoreboardName), sb);
        }
     ...
     */
    public @Nullable dev.havoc.havoctab.api.scoreboard.Scoreboard detectHighestScoreboard(@NonNull TabPlayer p) {
    ...
    public void onDisplayObjective(@NotNull TabPlayer receiver, int slot, @Nullable String objective) {
        ...
        if (objective == null) {
            ...
            HavocTab.getInstance().debug("Player " + receiver.getName() + " received scoreboard called " + objective + ", disabling HavocTab's scoreboard slotting.");
    ...
    public void onObjective(@NotNull TabPlayer receiver, int action, @NotNull String objective) {
        if (action == Scoreboard.ObjectiveAction.UNREGISTER && objective.equals(receiver.scoreboardData.otherPluginScoreboard)) {
            HavocTab.getInstance().debug("Player " + receiver.getName() + " no longer has another scoreboard, slotting HavocTab scoreboard.");
            receiver.scoreboardData.otherPluginScoreboard = null;
            if (receiver.scoreboardData.activeScoreboard != null) {
                receiver.getScoreboard().setDisplaySlot(OBJECTIVE_NAME, Scoreboard.DisplaySlot.SIDEBAR);
            }
    ...
    @Override
    @NotNull
    public dev.havoc.havoctab.api.scoreboard.Scoreboard createScoreboard(@NonNull String name, @NonNull String title, @NonNull List<String> lines) {
        ensureActive();
    ...
    @Override
    public void removeScoreboard(@NonNull dev.havoc.havoctab.api.scoreboard.Scoreboard scoreboard) {
    ...
    @Override
    public void showScoreboard(@NonNull dev.havoc.havoctab.api.TabPlayer player, @NonNull dev.havoc.havoctab.api.scoreboard.Scoreboard scoreboard) {
        ensureActive();
    ...
    @Override
    public boolean hasCustomScoreboard(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
        ensureActive();
        return ((TabPlayer)player).scoreboardData.forcedScoreboard != null;
    }
    
    @Override
    public void resetScoreboard(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
        ensureActive();
        TabPlayer p = (TabPlayer) player;
        ...
        customThread.execute(new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> {
            if (p.scoreboardData.forcedScoreboard != null) {
                p.scoreboardData.forcedScoreboard.removePlayer(p);
                p.scoreboardData.forcedScoreboard = null;
                dev.havoc.havoctab.api.scoreboard.Scoreboard sb = detectHighestScoreboard(p);
    ...
    @Override
    public boolean hasScoreboardVisible(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
        ensureActive();
        return ((TabPlayer)player).scoreboardData.visible;
    }

    @Override
    public void setScoreboardVisible(@NonNull dev.havoc.havoctab.api.TabPlayer p, boolean visible, boolean sendToggleMessage) {
        ensureActive();
        ...
        if (visible) {
            ...
            if (sendToggleMessage) {
                player.sendMessage(HavocTab.getInstance().getConfiguration().getMessages().getScoreboardOn());
            }
            ...
            if (sendToggleMessage) {
                player.sendMessage(HavocTab.getInstance().getConfiguration().getMessages().getScoreboardOff());
            }
            if (toggleManager != null) {
                if (configuration.isHiddenByDefault()) {
                    toggleManager.remove(player);
    ...
    @Override
    public void toggleScoreboard(@NonNull dev.havoc.havoctab.api.TabPlayer player, boolean sendToggleMessage) {
        ensureActive();
    ...
    @Override
    @Nullable
    public ScoreboardImpl getActiveScoreboard(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
        ensureActive();
    ...
    @NotNull
    public Map<String, dev.havoc.havoctab.api.scoreboard.Scoreboard> getRegisteredScoreboards() {
