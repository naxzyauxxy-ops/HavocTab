package dev.havoc.havoctab.shared.data;
...
public class Server {
    ...
    Server(@NonNull String name) {
        this.name = name;
        serverGroup = HavocTab.getInstance().getDataManager().computeServerGroup(this);
    }
    ...
    public static Server byName(@Nullable String name) {
        ...
        return HavocTab.getInstance().getDataManager().getServers().computeIfAbsent(name, Server::new);
    }
    ...
    public boolean canSee(@NotNull Server other) {
        ...
        if (HavocTab.getInstance().getConfiguration().getConfig().getGlobalPlayerList() != null) {
