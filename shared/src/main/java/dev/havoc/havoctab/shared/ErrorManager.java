...
public class ErrorManager {
            ...
            try (BufferedWriter buf = new BufferedWriter(new FileWriter(file, true))) {
                if (message != null) {
                    if (file.length() < TabConstants.MAX_LOG_SIZE)
                        buf.write(dateFormat.format(new Date()) + "[HavocTab v" + ProjectVariables.PLUGIN_VERSION + "] " + message + System.lineSeparator());
                    if (intoConsoleToo || HavocTab.getInstance().getConfiguration().getConfig().isDebugMode())
                        HavocTab.getInstance().getPlatform().logWarn(new TabTextComponent(message, TabTextColor.RED));
                }
                for (String line : error) {
                    if (file.length() < TabConstants.MAX_LOG_SIZE)
                        buf.write(dateFormat.format(new Date()) + line + System.lineSeparator());
                    if (intoConsoleToo || HavocTab.getInstance().getConfiguration().getConfig().isDebugMode())
                        HavocTab.getInstance().getPlatform().logWarn(new TabTextComponent(line, TabTextColor.RED));
                }
            }
    ...
    /**
     * Prints error message if errors were thrown when firing a HavocTab event.
     *
     * @param   event
    ...
    public void redisBungeeRegisterFail(@NotNull Exception e) {
        printError("Failed to register HavocTab channel in RedisBungee due to an error ", e, false, errorLog);
    }
