...
public class PlaceholdersConfiguration {
    ...
    private static SimpleDateFormat parseDateFormat(@NonNull String value, @NonNull String defaultValue, @NonNull Locale locale) {
        try {
            ...
            HavocTab.getInstance().getConfigHelper().startup().startupWarn("Format \"" + value + "\" is not a valid date/time format. Did you try to use color codes?");
    ...
    private static Locale parseLocaleTag(@NonNull String value) {
        ...
        if (locale.getLanguage().isEmpty()) {
            HavocTab.getInstance().getConfigHelper().startup().startupWarn("Locale \"" + value + "\" is not valid, see https://wikipedia.org/wiki/IETF_language_tag");
