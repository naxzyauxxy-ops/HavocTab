package dev.havoc.havoctab.shared.config.mysql;
...
public class MySQLGroupConfiguration implements PropertyConfiguration {
    ...
    public MySQLGroupConfiguration(@NonNull MySQL mysql) throws SQLException {
        ...
        if (crs.size() == 0) {
            HavocTab.getInstance().getConfigHelper().startup().startupWarn("[MySQL] Using MySQL to store groups and users, however, the database is empty. " +
                    "You can get started by uploading existing data in files using \"/" + HavocTab.getInstance().getPlatform().getCommand() + " mysql upload\". Further modifications can " +
                    "be done using property commands (/" + HavocTab.getInstance().getPlatform().getCommand() + " <group / player> <name> <property> <value...>).");
            return;
        }
        while (crs.next()) {
