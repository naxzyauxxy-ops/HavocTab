...
public class RelationalPlaceholderImpl extends TabPlaceholder implements RelationalPlaceholder {
    ...
    @NonNull private final BiFunction<dev.havoc.havoctab.api.TabPlayer, dev.havoc.havoctab.api.TabPlayer, String> function;
    ...
    @Override
    public void update(@NonNull dev.havoc.havoctab.api.TabPlayer viewer, @NonNull dev.havoc.havoctab.api.TabPlayer target) {
        updateValue(viewer, target, request((TabPlayer) viewer, (TabPlayer) target));
    }

    @Override
    public void updateValue(@NonNull dev.havoc.havoctab.api.TabPlayer viewer, @NonNull dev.havoc.havoctab.api.TabPlayer target, @Nullable String value) {
        if (hasValueChanged((TabPlayer) viewer, (TabPlayer) target, value)) {
            for (RefreshableFeature r : getUsedByFeatures()) {
                TimedCaughtTask task = new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> r.refresh((TabPlayer) target, true),
    ...
    public void updateFromNested(@NonNull TabPlayer viewer) {
        for (TabPlayer target : HavocTab.getInstance().getOnlinePlayers()) {
            ...
            for (RefreshableFeature f : getUsedByFeatures()) {
                TimedCaughtTask task = new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> f.refresh(target, true),
        ...
        for (RefreshableFeature f : getUsedByFeatures()) {
            TimedCaughtTask task = new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> f.refresh(viewer, true),
                    f.getFeatureName(), f.getRefreshDisplayName());
            if (f instanceof CustomThreaded) {
                ((CustomThreaded) f).getCustomThread().execute(task);
            } else {
    ...
    public @Nullable String request(@NonNull TabPlayer viewer, @NonNull TabPlayer target) {
        ...
        try {
            ...
            HavocTab.getInstance().getErrorManager().placeholderError("Relational placeholder " + identifier + " generated an error when setting for players " + viewer.getName() + " and " + target.getName(), t);
            return ERROR_VALUE;
        } finally {
            long timeDiff = System.currentTimeMillis() - time;
            if (timeDiff > TabConstants.Placeholder.RETURN_TIME_WARN_THRESHOLD) {
                HavocTab.getInstance().debug("Placeholder " + identifier + " took " + timeDiff + "ms to return value for " + viewer.getName() + " and " + target.getName());
            }
        }
    }
}