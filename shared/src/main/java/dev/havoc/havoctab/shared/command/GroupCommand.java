...
public class GroupCommand extends PropertyCommand {
    ...
    private void remove(@Nullable TabPlayer sender, @NotNull String group) {
        if (hasPermission(sender, TabConstants.Permission.COMMAND_DATA_REMOVE)) {
            HavocTab.getInstance().getConfiguration().getGroups().remove(group);
            for (TabPlayer pl : HavocTab.getInstance().getOnlinePlayers()) {
                if (pl.getGroup().equals(group) || TabConstants.DEFAULT_GROUP.equals(group)) {
                    HavocTab.getInstance().getFeatureManager().onGroupChange(pl);
                }
            }
    ...
    private void sendGroupInfo(@Nullable TabPlayer sender, @NotNull String group) {
        ...
        for (Map.Entry<String, Object> entry : HavocTab.getInstance().getConfiguration().getGroups().getGlobalSettings(group).entrySet()) {
            sendMessage(sender, "  " + entry.getKey() + ": " + entry.getValue());
        }
        for (Map.Entry<String, Map<String, Object>> entry : HavocTab.getInstance().getConfiguration().getGroups().getPerWorldSettings(group).entrySet()) {
        ...
        for (Map.Entry<String, Map<String, Object>> entry : HavocTab.getInstance().getConfiguration().getGroups().getPerServerSettings(group).entrySet()) {
            if (entry.getValue() == null) continue;
    ...
    @Override
    public void saveEntity(@Nullable TabPlayer sender, @NotNull String group, @NotNull String type, @NotNull String value, @Nullable Server server, @Nullable World world) {
        if (!value.isEmpty()) {
        ...
        String[] property = HavocTab.getInstance().getConfiguration().getGroups().getProperty(group, type, server, world);
        if (property.length > 0 && String.valueOf(value.isEmpty() ? null : value).equals(String.valueOf(property[0]))) return;
        HavocTab.getInstance().getConfiguration().getGroups().setProperty(group, type, server, world, value.isEmpty() ? null : value);
        for (TabPlayer pl : HavocTab.getInstance().getOnlinePlayers()) {
            if (pl.getGroup().equals(group) || TabConstants.DEFAULT_GROUP.equals(group)) {
                HavocTab.getInstance().getFeatureManager().onGroupChange(pl);
            }
    ...
    public @NotNull List<String> complete(@Nullable TabPlayer sender, @NotNull String[] arguments) {
        if (arguments.length == 1) {
            Set<String> groups = new HashSet<>(HavocTab.getInstance().getConfiguration().getGroups().getAllEntries());
