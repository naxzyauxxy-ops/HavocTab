...
import dev.havoc.havoctab.shared.features.proxy.message.*;
...
public abstract class ProxySupport extends TabFeature implements JoinListener, QuitListener,
    ...
    protected ProxySupport(@NotNull String channelName) {
        ...
        HavocTab.getInstance().debug("[Proxy Support] Using channel name: " + channelName);
    }
    ...
    public synchronized void processMessage(@NotNull String msg) {
        ...
        if (function == null) {
            HavocTab.getInstance().getErrorManager().unknownProxyMessage(action);
            return;
        }
        ProxyMessage proxyMessage;
        try {
            proxyMessage = function.apply(in);
            HavocTab.getInstance().debug("[Proxy Support] Decoded message " + proxyMessage);
        } catch (Exception e) {
            HavocTab.getInstance().getErrorManager().printError("Failed to decode proxy message \"" + new String(Base64.getDecoder().decode(msg)) + "\" ", e);
            return;
        }

        // Queue the task to make sure it does not execute before plugin fully loads, causing NPE
        HavocTab.getInstance().getCpu().runMeasuredTask(getFeatureName(), CpuUsageCategory.PROXY_MESSAGE, () -> {
            if (proxyMessage.getCustomThread() != null) {
                proxyMessage.getCustomThread().execute(new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> proxyMessage.process(this), getFeatureName(), CpuUsageCategory.PROXY_MESSAGE));
            } else {
    ...
    public void load() {
        register();
        for (TabPlayer p : HavocTab.getInstance().getOnlinePlayers()) onJoin(p);
        sendMessage(new LoadRequest());
    }

    @Override
    public void unload() {
        for (TabPlayer p : HavocTab.getInstance().getOnlinePlayers()) onQuit(p);
        unregister();
    }
    ...
    public void sendMessage(@NotNull ProxyMessage message) {
        ...
        HavocTab.getInstance().debug("[Proxy Support] Encoding message " + message);
        message.write(out);
