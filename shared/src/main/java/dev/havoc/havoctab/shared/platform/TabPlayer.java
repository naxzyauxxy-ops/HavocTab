...
import dev.havoc.havoctab.shared.event.impl.PlayerLoadEventImpl;
import dev.havoc.havoctab.shared.features.NickCompatibility;
import dev.havoc.havoctab.shared.features.belowname.BelowNamePlayerData;
import dev.havoc.havoctab.shared.features.chat.ChatPlayerData;
import dev.havoc.havoctab.shared.features.tags.TagPlayerData;
import dev.havoc.havoctab.shared.features.clientdisplay.ClientDisplayPlayerData;
import dev.havoc.havoctab.shared.features.bossbar.BossBarPlayerData;
import dev.havoc.havoctab.shared.features.header.HeaderFooterPlayerData;
import dev.havoc.havoctab.shared.features.layout.LayoutManagerImpl;
import dev.havoc.havoctab.shared.features.nametags.NameTagPlayerData;
import dev.havoc.havoctab.shared.features.playerlist.TablistFormattingPlayerData;
import dev.havoc.havoctab.shared.features.playerlistobjective.PlayerListObjectivePlayerData;
import dev.havoc.havoctab.shared.features.scoreboard.ScoreboardPlayerData;
import dev.havoc.havoctab.shared.features.sorting.SortingPlayerData;
import dev.havoc.havoctab.shared.features.types.RefreshableFeature;
import dev.havoc.havoctab.shared.hook.FloodgateHook;
import dev.havoc.havoctab.shared.placeholders.expansion.ExpansionData;
 ...
 */
public abstract class TabPlayer implements dev.havoc.havoctab.api.TabPlayer {
    ...
    /**
     * World the player is currently in, {@code "N/A"} if HavocTab is
     * installed on proxy and bridge is not installed
     */
    @NotNull
    public World world;

    /** Server the player is currently in, {@code "N/A"} if HavocTab is installed on backend */
    @NotNull
    ...
    /** Data for HavocTab's client-side display toggles (ranks / belowname) */
    public final ClientDisplayPlayerData clientDisplayData = new ClientDisplayPlayerData();

    /** Data for HavocTab's chat feature (format toggle, public chat toggle, block list) */
    public final ChatPlayerData chatData = new ChatPlayerData();

    /** Data for HavocTab's cosmetic tags */
    public final TagPlayerData tagData = new TagPlayerData();
    ...
    protected TabPlayer(@NotNull Platform platform, @NotNull Object player, @NotNull UUID uniqueId, @NotNull String name,
        ...
        permissionGroup = HavocTab.getInstance().getGroupManager().detectPermissionGroup(this);
        ...
        tabListEntryTracker = HavocTab.getInstance().getTablistTrackers().get(uniqueId);
    }
    ...
    public void markAsLoaded(boolean join) {
        loaded = true;
        if (HavocTab.getInstance().getEventBus() != null) HavocTab.getInstance().getEventBus().fire(new PlayerLoadEventImpl(this, join));
    }
    ...
    public void setGroup(@NotNull String permissionGroup) {
        ...
        ((PlayerPlaceholder)HavocTab.getInstance().getPlaceholderManager().getPlaceholder(TabConstants.Placeholder.GROUP)).updateValue(this, getGroup());
        HavocTab.getInstance().getFeatureManager().onGroupChange(this);
    }
    ...
    public void setExpectedProfileName(@NonNull String profileName) {
        ...
        HavocTab.getInstance().debug("Changing expected profile name of player " + name + " from " + nickname + " to " + profileName + " as a result of an API call.");
        nickname = profileName;
        NickCompatibility nick = HavocTab.getInstance().getFeatureManager().getFeature(TabConstants.Feature.NICK_COMPATIBILITY);
    ...
    public Property loadPropertyFromConfig(@Nullable RefreshableFeature feature, @NotNull String property, @NotNull String ifNotSet) {
        String[] value = HavocTab.getInstance().getConfiguration().getUsers().getProperty(name, property, server, world);
        if (value.length == 0) {
            value = HavocTab.getInstance().getConfiguration().getUsers().getProperty(uniqueId.toString(), property, server, world);
        }
        if (value.length == 0) {
            value = HavocTab.getInstance().getConfiguration().getGroups().getProperty(getGroup(), property, server, world);
        }
        if (value.length > 0) {
    ...
    public boolean updatePropertyFromConfig(@NotNull Property property, @NotNull String ifNotSet) {
        String[] value = HavocTab.getInstance().getConfiguration().getUsers().getProperty(name, property.getName(), server, world);
        if (value.length == 0) {
            value = HavocTab.getInstance().getConfiguration().getUsers().getProperty(uniqueId.toString(), property.getName(), server, world);
        }
        if (value.length == 0) {
            value = HavocTab.getInstance().getConfiguration().getGroups().getProperty(getGroup(), property.getName(), server, world);
        }
        if (value.length > 0) {
