...
public class PlayerPlaceholderImpl extends TabPlaceholder implements PlayerPlaceholder {
    ...
    @NonNull private final Function<dev.havoc.havoctab.api.TabPlayer, String> function;
     ...
     */
    public PlayerPlaceholderImpl(@NonNull String identifier, int refresh, @NonNull Function<dev.havoc.havoctab.api.TabPlayer, String> function) {
    ...
    @Override
    public void update(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
        updateValue(player, request((TabPlayer) player));
    }

    @Override
    public void updateValue(@NonNull dev.havoc.havoctab.api.TabPlayer player, @Nullable String value) {
        if (hasValueChanged((TabPlayer) player, value, true)) {
            ...
            for (RefreshableFeature r : getUsedByFeatures()) {
                TimedCaughtTask task = new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> r.refresh((TabPlayer) player, false),
    ...
    public static void bulkUpdateValues(@NotNull TabPlayer player, @NotNull Map<PlayerPlaceholderImpl, String> values) {
        ...
        for (RefreshableFeature r : features) {
            TimedCaughtTask task = new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> r.refresh(player, false),
                    r.getFeatureName(), r.getRefreshDisplayName());
            if (r instanceof CustomThreaded) {
                ((CustomThreaded) r).getCustomThread().execute(task);
            } else {
    ...
    public String request(@NonNull TabPlayer p) {
        ...
        try {
            ...
            HavocTab.getInstance().getErrorManager().placeholderError("Player placeholder " + identifier + " generated an error when setting for player " + p.getName(), t);
            return ERROR_VALUE;
        } finally {
            long timeDiff = System.currentTimeMillis() - time;
            if (timeDiff > TabConstants.Placeholder.RETURN_TIME_WARN_THRESHOLD) {
                HavocTab.getInstance().debug("Placeholder " + identifier + " took " + timeDiff + "ms to return value for player " + p.getName());
            }
        }
    }
}