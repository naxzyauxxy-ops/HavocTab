package dev.havoc.havoctab.shared.config.file;
...
public class YamlPropertyConfigurationFile extends YamlConfigurationFile implements PropertyConfiguration {
    ...
    public String[] getProperty(@NonNull String name, @NonNull String property, @Nullable Server server, @Nullable World world) {
        ...
        Object value;
        if ((value = getObject(new String[] {PER_WORLD, HavocTab.getInstance().getConfiguration().getGroup(worldGroups, worldName), name, property})) != null) {
            return new String[] {toString(value), category + "=" + name + ", world=" + worldName};
        }
        if ((value = getObject(new String[] {PER_WORLD, HavocTab.getInstance().getConfiguration().getGroup(worldGroups, worldName), TabConstants.DEFAULT_GROUP, property})) != null) {
            return new String[] {toString(value), category + "=" + TabConstants.DEFAULT_GROUP + ", world=" + worldName};
        }
        if ((value = getObject(new String[] {PER_SERVER, HavocTab.getInstance().getConfiguration().getServerGroup(serverGroups, server), name, property})) != null) {
            return new String[] {toString(value), category + "=" + name + ", server=" + serverName};
        }
        if ((value = getObject(new String[] {PER_SERVER, HavocTab.getInstance().getConfiguration().getServerGroup(serverGroups, server), TabConstants.DEFAULT_GROUP, property})) != null) {
