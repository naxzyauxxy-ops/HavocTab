...
public class MineSkin extends SkinSource {
    ...
    public Skin download(@NotNull String input) {
        try {
            ...
            HavocTab.getInstance().debug("Downloaded MINESKIN skin " + input + " in " + (System.currentTimeMillis()-time) + "ms");
            return new Skin(value, signature);
        } catch (IOException | ParseException e) {
            HavocTab.getInstance().getErrorManager().mineSkinDownloadError(input, e);
        }
        return null;
    }
}