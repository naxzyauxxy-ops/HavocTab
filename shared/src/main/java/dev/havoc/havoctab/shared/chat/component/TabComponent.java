package dev.havoc.havoctab.shared.chat.component;
...
import dev.havoc.havoctab.shared.chat.rgb.RGBUtils;
...
public abstract class TabComponent {
    ...
    /** Formatter to convert gradient into HavocTab's #RRGGBB spam */
    ...
    /** Formatter to convert RGB code to use HavocTab's #RRGGBB */
    ...
    static {
        ...
        OBJECT_PATTERNS.put(Pattern.compile("<head:([^>:]+)(?::([^>]+))?>"), matcher -> {
            try {
                ...
                if (HavocTab.getInstance().getConfiguration().getConfig().getComponents().isDisableShadowForHeads()) {
        ...
        OBJECT_PATTERNS.put(Pattern.compile("<mineskin:([^>:]+)>"), matcher -> {
            TabList.Skin skin = HavocTab.getInstance().getConfiguration().getSkinManager().getSkin("mineskin:" + matcher.group(1));
            if (skin != null) {
    ...
    public <T> T convert() {
        if (converted == null) converted = HavocTab.getInstance().getPlatform().convertComponent(this);
        return (T) converted;
    }
