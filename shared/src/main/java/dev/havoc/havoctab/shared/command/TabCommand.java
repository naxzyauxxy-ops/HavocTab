...
import dev.havoc.havoctab.shared.command.bossbar.BossBarCommand;
import dev.havoc.havoctab.shared.command.scoreboard.ScoreboardCommand;
...
public class TabCommand extends SubCommand {
    ...
    private void help(@Nullable TabPlayer sender) {
        if (hasPermission(sender, TabConstants.Permission.COMMAND_ALL)) {
            sendMessage(sender, "&3HavocTab v" + ProjectVariables.PLUGIN_VERSION);
            for (String message : getMessages().getHelpMenu()) {
                sendMessage(sender, message.replace("/tab", "/" + HavocTab.getInstance().getPlatform().getCommand()));
            }
        }
    }
