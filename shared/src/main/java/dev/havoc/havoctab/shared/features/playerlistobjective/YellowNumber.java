...
public class YellowNumber extends RefreshableFeature implements JoinListener, QuitListener, Loadable,
    ...
    public static final String OBJECTIVE_NAME = "HTab-PlayerList";

    private final StringToComponentCache cache = new StringToComponentCache("Playerlist Objective", 1000);

    private final ThreadExecutor customThread = new ThreadExecutor("HavocTab Playerlist Objective Thread");
    ...
    public YellowNumber(@NotNull PlayerListObjectiveConfiguration configuration) {
        ...
        disableChecker = new DisableChecker(this, HavocTab.getInstance().getPlaceholderManager().getConditionManager().getByNameOrExpression(configuration.getDisableCondition()), this::onDisableConditionChange, p -> p.playerlistObjectiveData.disabled);
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.YELLOW_NUMBER + "-Condition", disableChecker);
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.YELLOW_NUMBER_TEXT, titleRefresher);
        if (proxy != null) {
    ...
    public int getValue(@NotNull TabPlayer p) {
        ...
        try {
            ...
            try {
                ...
                // Float
                HavocTab.getInstance().getConfigHelper().runtime().floatInPlayerlistObjective(p, configuration.getValue(), string);
                return value;
            } catch (NumberFormatException e2) {
                // Not a float (invalid)
                HavocTab.getInstance().getConfigHelper().runtime().invalidNumberForPlayerlistObjective(p, configuration.getValue(), string);
                return 0;
            }
        }
    }
