...
public class Permissions extends SortingType {
    ...
    public Permissions(Sorting sorting, String options) {
        ...
        for (String permission : sortedGroups.keySet()) {
            PlayerPlaceholderImpl pl = HavocTab.getInstance().getPlaceholderManager().registerPlayerPlaceholder("%permission:" + permission + "%",
    ...
    public String getChars(@NotNull TabPlayer p) {
        ...
        if (position == 0) {
            HavocTab.getInstance().getConfigHelper().runtime().noPermissionFromSortingList(sortedGroups.keySet(), p);
