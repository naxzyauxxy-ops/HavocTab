...
import dev.havoc.havoctab.shared.proxy.message.outgoing.PermissionRequest;
import dev.havoc.havoctab.shared.proxy.message.outgoing.PlayerJoin;
import dev.havoc.havoctab.shared.task.PluginMessageEncodeTask;
...
public abstract class ProxyTabPlayer extends TabPlayer {
    ...
    protected ProxyTabPlayer(@NotNull ProxyPlatform platform, @NotNull Object player, @NotNull UUID uniqueId,
        ...
        super(platform, player, uniqueId, name, server, "N/A", protocolVersion, HavocTab.getInstance().getConfiguration().getConfig().isOnlineUuidInTabList());
    ...
    public void sendJoinPluginMessage() {
        ...
        sendPluginMessage(new PlayerJoin(
                HavocTab.getInstance().getGroupManager().getPermissionPlugin().contains("Vault") &&
                    !HavocTab.getInstance().getConfiguration().getConfig().isGroupsByPermissions(),
                HavocTab.getInstance().getPlaceholderManager().getBridgePlaceholders(),
                HavocTab.getInstance().getConfiguration().getConfig().getReplacements().getValues()
        ));
    ...
    public void setGamemode(int gamemode) {
        ...
        ((PlayerPlaceholder) HavocTab.getInstance().getPlaceholderManager().getPlaceholder(TabConstants.Placeholder.GAMEMODE)).update(this);
        HavocTab.getInstance().getFeatureManager().onGameModeChange(this);
    }
    ...
    public void setInvisibilityPotion(boolean invisibilityPotion) {
        ...
        ((PlayerPlaceholder) HavocTab.getInstance().getPlaceholderManager().getPlaceholder(TabConstants.Placeholder.INVISIBLE)).update(this);
    }
    ...
    public boolean hasPermission(@NotNull String permission) {
        if (HavocTab.getInstance().getConfiguration().getConfig().isBukkitPermissions()) {
