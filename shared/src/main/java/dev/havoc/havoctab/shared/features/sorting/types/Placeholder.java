...
public class Placeholder extends SortingType {
    ...
    public static PlaceholderSplitResult splitValue(@NotNull String options) {
        ...
        if (!matcher.matches()) {
            HavocTab.getInstance().getConfigHelper().startup().invalidSortingLine("PLACEHOLDER:" + options, "Invalid format. Expected \"%placeholder%:value1,value2,...,valueN\".");
            return null;
        }
    ...
    public String getChars(@NotNull TabPlayer p) {
        ...
        if (!sortingMap.containsKey(cleanOutput)) {
            HavocTab.getInstance().getConfigHelper().runtime().valueNotInPredefinedValues(sortingPlaceholder.getIdentifier(), sortingMap.keySet(), cleanOutput, p);
