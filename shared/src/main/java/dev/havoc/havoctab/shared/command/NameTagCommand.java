...
import dev.havoc.havoctab.shared.features.nametags.NameTagInvisibilityReason;
...
public class NameTagCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        ...
        NameTag teams = HavocTab.getInstance().getNameTagManager();
        if (teams == null) {
    ...
    private void processTarget(@NotNull NameTag teams, @Nullable TabPlayer sender, @NotNull String action, @NotNull String[] args) {
        ...
        TabPlayer player = effectiveArgs.length >= 1 ? HavocTab.getInstance().getPlayer(effectiveArgs[0]) : sender;
        if (player == null) {
        ...
        TabPlayer viewer = effectiveArgs.length >= 2 ? HavocTab.getInstance().getPlayer(effectiveArgs[1]) : null;
    ...
    private void processView(@NotNull NameTag teams, @Nullable TabPlayer sender, @NotNull String action, @NotNull String[] args) {
        boolean silent = args.length > 0 && args[args.length - 1].equals("-s");
        String[] effectiveArgs = silent ? Arrays.copyOf(args, args.length - 1) : args;
        TabPlayer viewer = effectiveArgs.length >= 1 ? HavocTab.getInstance().getPlayer(effectiveArgs[0]) : sender;
