...
import dev.havoc.havoctab.shared.features.tablistpages.TablistPages;
...
public class FeatureManager {
    ...
    public void loadFeaturesFromConfig() {
        ...
        if (config.isTagsEnabled()) {
            try {
                registerFeature(TabConstants.Feature.TAGS, new TagManager());
            } catch (java.io.IOException e) {
                HavocTab.getInstance().getConfigHelper().startup().startupWarn(
                        "Failed to load tags.yml, the tags feature is disabled: " + e.getMessage());
            }
        }
        ...
        // Registered late: pagination only flips the listed flag, and should run after
        // the features that create and format tablist entries.
        if (config.getTablistPages() != null) {
            registerFeature(TabConstants.Feature.TABLIST_PAGES, new TablistPages(config.getTablistPages()));
        }
