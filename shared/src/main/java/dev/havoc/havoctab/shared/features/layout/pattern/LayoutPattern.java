package dev.havoc.havoctab.shared.features.layout.pattern;
...
public class LayoutPattern extends RefreshableFeature implements Layout, Conditional {
    ...
    public LayoutPattern(@NotNull LayoutManagerImpl manager, @NotNull LayoutDefinition def) {
        ...
        name = def.getName();
        displayCondition = HavocTab.getInstance().getPlaceholderManager().getConditionManager().getByNameOrExpression(def.getCondition());
    ...
    public void addGroup(@Nullable String condition, int[] slots) {
        ...
        if (condition != null) {
            Condition compiled = HavocTab.getInstance().getPlaceholderManager().getConditionManager().getByNameOrExpression(condition);
