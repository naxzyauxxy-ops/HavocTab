...
public class PlayerJoin extends ProxyMessage {
    ...
    public void process(@NotNull ProxySupport proxySupport) {
        ...
        if (proxySupport.getProxyPlayers().containsKey(decodedPlayer.getUniqueId())) {
            HavocTab.getInstance().debug("[Proxy Support] The proxy player " + decodedPlayer.getName() + " is already connected, cannot process join.");
            return;
        }
        ...
        if (HavocTab.getInstance().getPlayer(decodedPlayer.getUniqueId()) == null) {
            HavocTab.getInstance().getFeatureManager().onJoin(decodedPlayer);
        }
    }
}
