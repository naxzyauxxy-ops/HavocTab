...
import dev.havoc.havoctab.shared.config.PropertyConfiguration;
...
import dev.havoc.havoctab.shared.config.Configs;
import dev.havoc.havoctab.shared.config.mysql.MySQL;
import dev.havoc.havoctab.shared.config.file.YamlPropertyConfigurationFile;
...
public class MySQLCommand extends SubCommand {
    ...
    private void download(@Nullable TabPlayer sender) {
        MySQL mysql = HavocTab.getInstance().getConfiguration().getMysql();
        if (mysql == null) {
            sendMessage(sender, getMessages().getMySQLFailNotEnabled());
            return;
        }
        HavocTab.getInstance().getCPUManager().getMysqlThread().execute(() -> {
            try {
                YamlPropertyConfigurationFile groupFile = new YamlPropertyConfigurationFile(Configs.class.getClassLoader().getResourceAsStream("config/groups.yml"), new File(HavocTab.getInstance().getDataFolder(), "groups.yml"));
                YamlPropertyConfigurationFile userFile = new YamlPropertyConfigurationFile(Configs.class.getClassLoader().getResourceAsStream("config/users.yml"), new File(HavocTab.getInstance().getDataFolder(), "users.yml"));
                ...
                HavocTab.getInstance().getErrorManager().criticalError("MySQL download failed", e);
            }
        });
    }
    ...
    private void upload(@Nullable TabPlayer sender) {
        ...
        HavocTab.getInstance().getCPUManager().getMysqlThread().execute(() -> {
            try {
                ...
                upload(groupFile, HavocTab.getInstance().getConfiguration().getGroups());
                upload(userFile, HavocTab.getInstance().getConfiguration().getUsers());
                sendMessage(sender, getMessages().getMySQLUploadSuccess());
            } catch (YAMLException | IOException | SQLException e) {
                sendMessage(sender, getMessages().getMySQLFailError());
                HavocTab.getInstance().getErrorManager().criticalError("MySQL upload failed", e);
            }
        });
    }
