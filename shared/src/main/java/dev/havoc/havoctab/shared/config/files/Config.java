...
import dev.havoc.havoctab.shared.chat.ComponentConfiguration;
import dev.havoc.havoctab.shared.config.converter.LegacyConverter;
import dev.havoc.havoctab.shared.config.converter.ModernConverter;
...
import dev.havoc.havoctab.shared.config.mysql.MySQLConfiguration;
import dev.havoc.havoctab.shared.features.PerWorldPlayerListConfiguration;
import dev.havoc.havoctab.shared.features.belowname.BelowNameConfiguration;
import dev.havoc.havoctab.shared.features.bossbar.BossBarConfiguration;
import dev.havoc.havoctab.shared.features.chat.ChatConfiguration;
import dev.havoc.havoctab.shared.features.clientdisplay.ClientDisplayConfiguration;
import dev.havoc.havoctab.shared.features.tablistpages.TablistPagesConfiguration;
import dev.havoc.havoctab.shared.features.globalplayerlist.GlobalPlayerListConfiguration;
import dev.havoc.havoctab.shared.features.header.HeaderFooterConfiguration;
import dev.havoc.havoctab.shared.features.layout.LayoutConfiguration;
import dev.havoc.havoctab.shared.features.nametags.TeamConfiguration;
import dev.havoc.havoctab.shared.features.pingspoof.PingSpoofConfiguration;
import dev.havoc.havoctab.shared.features.playerlist.TablistFormattingConfiguration;
import dev.havoc.havoctab.shared.features.playerlistobjective.PlayerListObjectiveConfiguration;
import dev.havoc.havoctab.shared.features.proxy.ProxySupportConfiguration;
import dev.havoc.havoctab.shared.features.scoreboard.ScoreboardConfiguration;
import dev.havoc.havoctab.shared.features.sorting.SortingConfiguration;
import dev.havoc.havoctab.shared.placeholders.PlaceholderRefreshConfiguration;
import dev.havoc.havoctab.shared.placeholders.PlaceholderReplacementsConfiguration;
import dev.havoc.havoctab.shared.placeholders.PlaceholdersConfiguration;
...
public class Config {
    ...
    @NotNull private final ConfigurationFile config = new YamlConfigurationFile(getClass().getClassLoader().getResourceAsStream("config/config.yml"),
            new File(HavocTab.getInstance().getDataFolder(), "config.yml"));
    ...
    @NotNull private final ClientDisplayConfiguration clientDisplay;
    @Nullable private ChatConfiguration chat;
    @Nullable private TablistPagesConfiguration tablistPages;

    private final boolean tagsEnabled = config.getBoolean("tags.enabled", false);
    private final boolean preventSpectatorEffect = config.getBoolean("prevent-spectator-effect.enabled", false);
    private final boolean bukkitPermissions = HavocTab.getInstance().getPlatform().isProxy() && config.getBoolean("use-bukkit-permissions-manager", false);
    ...
    private final boolean packetEventsCompensation = config.getBoolean("compensate-for-packetevents-bug", false) && !HavocTab.getInstance().getPlatform().isSafeFromPacketEventsBug();
    ...
    public Config() throws IOException {
        ...
        clientDisplay = ClientDisplayConfiguration.fromSection(config.getConfigurationSection("client-display-settings"));
        if (config.getBoolean("chat.enabled", false)) chat = ChatConfiguration.fromSection(config.getConfigurationSection("chat"));
        if (config.getBoolean("tablist-pages.enabled", false)) tablistPages = TablistPagesConfiguration.fromSection(config.getConfigurationSection("tablist-pages"));
        ...
        if (layout != null) {
            if (perWorldPlayerList != null) {
                HavocTab.getInstance().getConfigHelper().startup().startupWarn(config.getFile(), "Both per world playerlist and layout features are enabled, but layout makes per world playerlist redundant. " +
            ...
            if (playerlistObjective != null) {
                HavocTab.getInstance().getConfigHelper().startup().startupWarn(config.getFile(), "Layout feature breaks playerlist-objective feature, because it replaces real player with fake slots " +
            ...
            if (preventSpectatorEffect) {
                HavocTab.getInstance().getConfigHelper().hint(config.getFile(), "Layout feature automatically includes prevent-spectator-effect, therefore the feature can be disabled " +
            ...
            if (globalPlayerList != null) {
                HavocTab.getInstance().getConfigHelper().startup().startupWarn(config.getFile(), "Both global playerlist and layout features are enabled, but layout makes global playerlist redundant. " +
