...
public class NameTag extends TabFeature implements NameTagManager, JoinListener, QuitListener,
    ...
    public String getDisplayedSuffix(@NonNull TabPlayer p, @NonNull TabPlayer viewer) {
        if (viewer.clientDisplayData.hidesNameTagRanks()) {
            return TagManager.decorate(p, "", TagManager.Slot.NAMETAG);
        }
        return TagManager.decorate(p, p.teamData.suffix.getFormat(viewer), TagManager.Slot.NAMETAG);
    }
    ...
    /**
     * Resends one player's team prefix and suffix to everyone who can see them. Used when
     * HavocTab's tags feature changes what a player is wearing, since the tag is appended at
     * render time and so no property changed to trigger the usual refresh.
     *
     * @param   player
     *          Player whose prefix and suffix should be resent
     */
    public void refreshPrefixSuffixForPlayer(@NonNull TabPlayer player) {
        customThread.execute(new TimedCaughtTask(HavocTab.getInstance().getCpu(), () -> {
            if (onlinePlayers == null) return; // Feature not loaded yet
            if (player.teamData.teamName == null) return;
            for (TabPlayer viewer : onlinePlayers.getPlayers()) {
                if (!viewer.teamData.hasTeamRegistered(player)) continue;
                String prefix = getDisplayedPrefix(player, viewer);
                String suffix = getDisplayedSuffix(player, viewer);
                viewer.getScoreboard().updateTeam(
        ...
        }, getFeatureName(), "Refreshing equipped tag"));
    }
