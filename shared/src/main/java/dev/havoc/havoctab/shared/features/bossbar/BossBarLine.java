...
public class BossBarLine implements BossBar {
    ...
    public BossBarLine(@NonNull BossBarManagerImpl manager, @NonNull String name, @NonNull BossBarConfiguration.BossBarDefinition configuration) {
        ...
        this.name = name;
        displayCondition = HavocTab.getInstance().getPlaceholderManager().getConditionManager().getByNameOrExpression(configuration.getDisplayCondition());
        ...
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.bossBarTitle(name),
                textRefresher = new TextRefresher());
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.bossBarProgress(name),
                progressRefresher = new ProgressRefresher());
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.bossBarColor(name),
                colorRefresher = new ColorRefresher());
        HavocTab.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.bossBarStyle(name),
                styleRefresher = new StyleRefresher());
        announceEndPlaceholder = HavocTab.getInstance().getPlaceholderManager().registerServerPlaceholder(
                TabConstants.Placeholder.bossbarAnnounceTotal(name), -1, () -> PerformanceUtil.toString(announceTimeTotalSeconds));
        HavocTab.getInstance().getPlaceholderManager().registerServerPlaceholder(
    ...
    public BarColor parseColor(@NotNull TabPlayer player, @NonNull String color) {
        try {
            ...
            HavocTab.getInstance().getConfigHelper().runtime().invalidBossBarProperty(
                    this,
                    color,
    ...
    @Override
    public void addPlayer(@NonNull dev.havoc.havoctab.api.TabPlayer p) {
    ...
    @Override
    public void removePlayer(@NonNull dev.havoc.havoctab.api.TabPlayer p) {
        TabPlayer player = (TabPlayer) p;
        if (!player.bossbarData.visibleBossBars.containsKey(this)) return;
        players.remove(player);
    ...
    @Override
    @NotNull
    public List<dev.havoc.havoctab.api.TabPlayer> getPlayers() {
        return new ArrayList<>(players);
    }

    @Override
    public boolean containsPlayer(@NonNull dev.havoc.havoctab.api.TabPlayer player) {
