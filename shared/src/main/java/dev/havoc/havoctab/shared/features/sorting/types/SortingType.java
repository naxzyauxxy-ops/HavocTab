...
public abstract class SortingType {
    ...
    protected SortingType(@NotNull Sorting sorting, @NotNull String displayName, @NotNull String sortingPlaceholder) {
        ...
        if (!sortingPlaceholder.startsWith("%") || !sortingPlaceholder.endsWith("%")) {
            HavocTab.getInstance().getConfigHelper().startup().invalidSortingPlaceholder(sortingPlaceholder, this);
        } else {
            sorting.addUsedPlaceholder(sortingPlaceholder);
            this.sortingPlaceholder = HavocTab.getInstance().getPlaceholderManager().getPlaceholderReference(sortingPlaceholder);
        }
    }
    ...
    public double parseDouble(@NotNull String output, double defaultValue, @NotNull TabPlayer player) {
        try {
            ...
            HavocTab.getInstance().getConfigHelper().runtime().invalidInputForNumericSorting(this, sortingPlaceholder.getIdentifier(), output, player);
            return defaultValue;
        }
    }
