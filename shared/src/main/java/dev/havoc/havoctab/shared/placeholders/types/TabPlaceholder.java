...
import dev.havoc.havoctab.shared.placeholders.PlaceholderReplacementPattern;
...
public abstract class TabPlaceholder implements Placeholder {
    ...
    protected TabPlaceholder(@NonNull String identifier, int refresh) {
        ...
        Map<Object, Object> map = HavocTab.getInstance().getConfiguration().getConfig().getReplacements().getValues().get(identifier);
        ...
        for (String nested : replacements.getNestedPlaceholders()) {
            HavocTab.getInstance().getPlaceholderManager().getPlaceholder(nested).addParent(this);
        }
    }
    ...
    protected String setPlaceholders(@NonNull String value, @Nullable TabPlayer player) {
        ...
        for (String s : PlaceholderManagerImpl.detectPlaceholders(string)) {
            ...
            TabPlaceholder nested = HavocTab.getInstance().getPlaceholderManager().getPlaceholder(s);
