...
/** Applies HavocTab's player-info anti-override rules through Fand's packet API. */
final class FandPacketListener {
    ...
    private static FandTabPlayer player(PacketController<?> packet) {
        ...
        if (viewer == null || HavocTab.getInstance() == null || HavocTab.getInstance().isPluginDisabled()) {
            return null;
        }
        var tabPlayer = HavocTab.getInstance().getPlayer(viewer.uniqueId());
