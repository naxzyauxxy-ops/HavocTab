...
public class NMSPacketScoreboard extends SafeScoreboard<BukkitTabPlayer> {
    ...
    public Object onPacketSend(@NonNull Object packet) {
        if (packet instanceof PacketPlayOutScoreboardDisplayObjective) {
            HavocTab.getInstance().getFeatureManager().onDisplayObjective(player, DisplayObjective_POSITION.getInt(packet),
                    (String) DisplayObjective_OBJECTIVE_NAME.get(packet));
        }
        if (packet instanceof PacketPlayOutScoreboardObjective) {
            HavocTab.getInstance().getFeatureManager().onObjective(player,
    ...
    private String maybeCut(@NonNull String string, int length) {
        if (player.getVersion().getMinorVersion() < 13 || HavocTab.getInstance().getConfiguration().getConfig().isPacketEventsCompensation()) {
