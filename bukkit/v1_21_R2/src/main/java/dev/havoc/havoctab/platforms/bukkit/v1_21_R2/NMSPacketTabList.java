package dev.havoc.havoctab.platforms.bukkit.v1_21_R2;
...
public class NMSPacketTabList extends TrackedTabList<BukkitTabPlayer> {
    ...
    public void updateDisplayName0(@NonNull UUID entry, @Nullable TabComponent displayName) {
        sendPacket(UPDATE_DISPLAY_NAME_SET, entry, "", null, false, 0, 0, displayName, 0);
    }

    @Override
    public void updateLatency(@NonNull UUID entry, int latency) {
        sendPacket(UPDATE_LATENCY_SET, entry, "", null, false, latency, 0, null, 0);
    }

    @Override
    public void updateGameMode(@NonNull UUID entry, int gameMode) {
        sendPacket(UPDATE_GAME_MODE_SET, entry, "", null, false, 0, gameMode, null, 0);
    }

    @Override
    public void updateListed(@NonNull UUID entry, boolean listed) {
        sendPacket(UPDATE_LISTED_SET, entry, "", null, listed, 0, 0, null, 0);
    }

    @Override
    public void updateListOrder(@NonNull UUID entry, int listOrder) {
        sendPacket(UPDATE_LIST_ORDER_SET, entry, "", null, false, 0, 0, null, listOrder);
    }
    ...
    public Object onPacketSend(@NonNull Object packet) {
        ...
        for (ClientboundPlayerInfoUpdatePacket.b nmsData : info.e()) {
            ...
            if (actions.contains(ADD_PLAYER)) {
                HavocTab.getInstance().getFeatureManager().onEntryAdd(player, profileId, nmsData.b().getName());
            }
