...
public class PerWorldPlayerList extends TabFeature implements Listener, Loadable, UnLoadable, VanishListener {
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        HavocTab.getInstance().getCPUManager().addTime(getFeatureName(), TabConstants.CpuUsageCategory.PLAYER_JOIN, System.nanoTime()-time);
    }
    ...
    public void onWorldChange(PlayerChangedWorldEvent e) {
        long time = System.nanoTime();
        checkPlayer(e.getPlayer());
        HavocTab.getInstance().getCPUManager().addTime(getFeatureName(), TabConstants.CpuUsageCategory.WORLD_SWITCH, System.nanoTime()-time);
    }
    ...
    private String computeWorldGroup(@NotNull Player p) {
        for (Entry<String, List<String>> group : configuration.getSharedWorlds().entrySet()) {
            ...
            for (String world : worlds) {
                if (HavocTab.getInstance().getDataManager().matchesPattern(p.getWorld().getName(), world)) {
                    return group.getKey();
                }
            }
        }
