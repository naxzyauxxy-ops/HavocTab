package dev.havoc.havoctab.platforms.bukkit.v1_20_R3;
...
public class NMSPacketScoreboard extends SafeScoreboard<BukkitTabPlayer> {
    ...
    public void setScore(@NonNull Score score) {
        sendPacket(new PacketPlayOutScoreboardScore(
                ...
                score.getNumberFormat() == null ? null : score.getNumberFormat().toFixedFormat(FixedFormat::new)
        ));
    }
    ...
    public Object onPacketSend(@NonNull Object packet) {
        if (packet instanceof PacketPlayOutScoreboardDisplayObjective display) {
            HavocTab.getInstance().getFeatureManager().onDisplayObjective(player, display.a().ordinal(), display.d());
        }
        if (packet instanceof PacketPlayOutScoreboardObjective objective) {
            HavocTab.getInstance().getFeatureManager().onObjective(player, objective.e(), objective.a());
        }
        if (packet instanceof PacketPlayOutScoreboardTeam team) {
            int action = getMethod(team);
            if (action != TeamAction.UPDATE) {
                Collection<String> players = team.f();
                if (players != null) {
                    String name = team.e();
    ...
    private int getMethod(@NonNull PacketPlayOutScoreboardTeam team) {
        if (team.d() == PacketPlayOutScoreboardTeam.a.a) {
            return 0;
        } else if (team.d() == PacketPlayOutScoreboardTeam.a.b) {
            return 1;
    ...
    private void sendPacket(@NotNull Packet<?> packet) {
        ((CraftPlayer)player.getPlayer()).getHandle().c.b(packet);
    }
}
