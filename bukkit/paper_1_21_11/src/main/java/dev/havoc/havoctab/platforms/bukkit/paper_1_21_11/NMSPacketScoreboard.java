package dev.havoc.havoctab.platforms.bukkit.paper_1_21_11;
...
public class NMSPacketScoreboard extends SafeScoreboard<BukkitTabPlayer> {
    ...
    private static final Constructor<ClientboundSetPlayerTeamPacket> teamConstructor;

    static {
        try {
            teamConstructor = ClientboundSetPlayerTeamPacket.class.getDeclaredConstructor(String.class, int.class, Optional.class, Collection.class);
    ...
    public Object onPacketSend(@NonNull Object packet) {
        if (packet instanceof ClientboundSetDisplayObjectivePacket display) {
            HavocTab.getInstance().getFeatureManager().onDisplayObjective(player, display.getSlot().ordinal(), display.getObjectiveName());
        }
        if (packet instanceof ClientboundSetObjectivePacket objective) {
            HavocTab.getInstance().getFeatureManager().onObjective(player, objective.getMethod(), objective.getObjectiveName());
        }
