...
/**
 * Main class for Forge HavocTab implementation.
 */
@OnlyIn(Dist.DEDICATED_SERVER)
@Mod(ProjectVariables.PLUGIN_ID)
public class ForgeHavocTab {
	 ...
	 */
	public ForgeHavocTab() {
		RegisterCommandsEvent.BUS.addListener(event -> COMMAND_DISPATCHER = event.getDispatcher());
		ServerStartingEvent.BUS.addListener(event -> HavocTab.create(new ForgePlatform(event.getServer())));
		ServerStoppingEvent.BUS.addListener(event -> HavocTab.getInstance().unload());
	}
